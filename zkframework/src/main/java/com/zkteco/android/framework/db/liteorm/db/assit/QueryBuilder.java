...
public class QueryBuilder<T> {
    ...
    public SQLStatement createStatement() {
        ...
        if (com.zkteco.android.framework.db.liteorm.db.assit.Checker.isEmpty(group) && !com.zkteco.android.framework.db.liteorm.db.assit.Checker.isEmpty(having)) {
            throw new IllegalArgumentException(
                    "HAVING仅允许在有GroupBy的时候使用(HAVING clauses are only permitted when using a groupBy clause)");
        }
        if (!com.zkteco.android.framework.db.liteorm.db.assit.Checker.isEmpty(limit) && !limitPattern.matcher(limit).matches()) {
            throw new IllegalArgumentException(
        ...
        if (!com.zkteco.android.framework.db.liteorm.db.assit.Checker.isEmpty(columns)) {
    ...
    private static void appendClause(StringBuilder s, String name, String clause) {
        if (!com.zkteco.android.framework.db.liteorm.db.assit.Checker.isEmpty(clause)) {
            s.append(name);
            s.append(clause);
        }
    }
